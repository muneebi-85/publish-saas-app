...
const INPUT =
  'h-11 w-full rounded-[10px] border border-ink-300 bg-white pl-10 pr-3 text-[14px] text-ink-900 ' +
  'placeholder:text-ink-400 transition-colors focus:border-red-brand focus:outline-none focus:ring-2 ' +
  'focus:ring-red-brand/50 disabled:cursor-not-allowed disabled:bg-ink-50 disabled:text-ink-400';
...
export const SubmitButton: React.FC<{
  ...
  <button
    ...
    className="flex h-11 w-full items-center justify-center gap-2 rounded-[10px] bg-red-brand text-[14px] font-semibold text-white transition-colors hover:bg-red-brand-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-brand/60 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60"
  >
...
export const FormError: React.FC<{ error: string | null }> = ({ error }) =>
  error ? (
    <p
      role="alert"
      className="mb-4 rounded-[10px] border border-crimson-200 bg-crimson-50 px-3.5 py-2.5 text-[13px] font-medium leading-relaxed text-crimson-700"
    >
      {error}
    </p>
  ) : null;
  ...
  <div className="space-y-2">
    {SOCIAL.map(({ strategy, name, Icon }) => (
      <button
        ...
        className="flex h-11 w-full items-center justify-center gap-2.5 rounded-[10px] border border-ink-300 bg-white text-[14px] font-medium text-ink-800 transition-colors hover:bg-ink-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink-900/40 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60"
      >
 ...
 * that the card is in the first HTML.
 *
 * The decision itself is `safeRedirect` (src/lib/oauth-return.ts) — the ONE
 * redirect guard in the codebase, unit-tested there. This used to carry its own
 * startsWith checks, which missed interior control characters: WHATWG URL
 * parsing strips tabs, so `/\t//evil.example` passed the local checks and
 * resolved to `https://evil.example/` once a router pushed it. The guards must
 * not drift, so there is only one implementation now.
 */
export function readRedirect(fallback = '/dashboard'): string {
  ...
  return safeRedirect(raw ?? undefined, fallback);
}
