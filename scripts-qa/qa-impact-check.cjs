...
async function main() {
  ...
  const r = await evalJs(`(() => {
    const impactEls = Array.from(document.querySelectorAll('div')).filter(d => {
      const s = getComputedStyle(d);
      return s.whitespace === 'nowrap' && d.scrollWidth > d.clientWidth + 100;
    }).map(d => ({
      cls: (d.className || '').toString().slice(0, 70),
      text: (d.textContent || '').trim().slice(0, 100),
      clientW: d.clientWidth, scrollW: d.scrollWidth,
      parentCls: d.parentElement ? (d.parentElement.className || '').toString().slice(0, 60) : ''
    }));
    return impactEls.slice(0, 6);
  })()`);
  console.log('NOWRAP OVERFLOW:', JSON.stringify(r, null, 1));
  ws.close();
  process.exit(0);
}
