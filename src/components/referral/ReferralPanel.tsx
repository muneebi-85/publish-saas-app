...
export function ReferralPanel() {
  ...
  useEffect(() => {
    void fetch('/api/referral')
      ...
      .then((data) => data && setStatus(data as ReferralStatus))
      .catch(() => setError('Could not load your referral link.'));
  }, []);

  const shareLink = status ? `${window.location.origin}/?ref=${status.code}` : '';
  ...
  return (
    <div className="space-y-5">
      {error && <p className="text-[12.5px] text-crimson-700 font-medium">{error}</p>}

      <div className="rounded-2xl border border-brand-600/20 bg-brand-600/[0.05] p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[12.5px] text-ink-600 mt-1 leading-relaxed">
              When a creator signs up through your link, you <strong>both</strong> get one free
              audit. No cap on total referrals — just share your link.
            </p>
          </div>
          <Badge variant="success" dot>
            {status ? `${status.credits} free audit${status.credits === 1 ? '' : 's'} earned` : '…'}
          </Badge>
        </div>
        ...
        {status && (
          <div className="mt-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2.5">
              <code className="flex-1 min-w-0 rounded-xl border border-white/[0.1] bg-surface-canvas px-3.5 py-2.5 text-[13px] font-mono text-ink-900 truncate">
                {shareLink}
              </code>
              ...
              {status.signups.map((s, i) => (
                <li key={i} className="flex items-center justify-between gap-3 px-5 py-3">
                  ...
                  <span className="text-[11.5px] text-ink-500">
                    ...
                    <span className={s.rewarded ? 'text-grass-700 font-medium' : 'text-ink-400'}>
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-surface-panel p-5">
        <div className="text-[13.5px] font-semibold text-ink-900">Got a referral code?</div>
        ...
        <form onSubmit={handleAttach} className="mt-3 flex flex-col sm:flex-row gap-2.5">
          <input
            ...
            className="flex-1 min-w-0 bg-white/[0.03] border border-white/[0.08] rounded-xl h-10 px-3.5 text-[13px] font-mono tracking-wider placeholder:text-ink-400 focus:border-brand-600 focus:ring-1 focus:ring-brand-600 transition-colors"
          />
